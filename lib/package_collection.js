...
PackageCollection.prototype = {
    ...
    collectPackages: function() {
        ...
        try {
            var jsonFile = JSON.parse(stripJsonComments(readFile(this.opts.paths.jsonFile, 'utf8'))),
                devDependencies = jsonFile.devDependencies || {},
        ...
        if (includeDev !== 'exclusive') {
            this.addDependencies(dependencies, jsonFile);
        }

        if (includeDev !== false) {
            this.addDependencies(devDependencies, jsonFile);
        }

        // Add the main files to project dependencies
        if (includeSelf !== false) {
            this.add(jsonFile.name || 'self', path.dirname(this.opts.paths.jsonFile), main);
        }
    },
     ...
     * @param jsonFile
     */
    addDependencies: function (dependencies, jsonFile) {
